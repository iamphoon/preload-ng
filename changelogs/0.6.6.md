...
| ------------- | --------------------------------- | ----------- | ------------------------------- |
| `readahead.c` | FD leak in `set_block()`          | 🔴 Critical | Added `close(fd)` on error path |
| `readahead.c` | Integer overflow in comparators   | 🔴 Critical | Safe comparison pattern         |
| `state.c`     | Copy-paste bug (line 228)         | 🔴 Critical | Fixed variable name             |
| `state.c`     | Memory leak in `read_map()`       | 🔴 Critical | Manual struct cleanup           |
| `state.c`     | FD leak in `preload_state_save()` | 🔴 Critical | Moved `close()` inside else     |
| `state.c`     | Suboptimal buffer allocation      | 🟡 Minor    | Increased initial size          |
| `proc.c`      | Memory leak in `proc_get_maps()`  | 🔴 Critical | Ownership tracking              |
